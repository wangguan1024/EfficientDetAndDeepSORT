...
class ModelInspector(object):
        ...
        for i in range(num_batches):
            ...
            for j in range(size_before_pad):
                img = driver.visualize(
                    raw_images[j], detections_bs[j], **kwargs)
                ...
                try:
                    ...
                    np.savetxt(output_tensor_path, detections_bs.shape[j])
                finally:
                    if f:
                        f.close()
    ...
    def saved_model_benchmark(self,
        ...
        raw_images = [np.array(Image.open(f))
                      for f in all_files[:self.batch_size]]
    ...
    def saved_model_video(self, video_input_path: Text, video_output_path: Text, **kwargs):
        ...
        while cap.isOpened():
            ...
            new_frame = driver.visualize(
                raw_frames[0], detections_bs[0], **kwargs)
    ...
    def build_and_save_model(self):
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            inputs = tf.placeholder(
                tf.float32, name='input', shape=self.inputs_shape)
    ...
    def freeze_model(self) -> Tuple[Text, Text]:
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            tf_graph = os.path.join(
                self.logdir, self.model_name + '_frozen.pb')
            tf.io.gfile.GFile(tf_graph, 'wb').write(
    ...
    def run_model(self, runmode, **kwargs):
        ...
        if runmode == 'dry':
            ...
            if kwargs.get('max_boxes_to_draw', None):
                config_dict['max_boxes_to_draw'] = kwargs.get(
                    'max_boxes_to_draw')
            if kwargs.get('min_score_thresh', None):
                config_dict['min_score_thresh'] = kwargs.get(
                    'min_score_thresh')
